...
def calculate_accuracy(cat_score, full_score):
    """Calculate the accuracy as a percentage of agreement between CAT score and full score."""
    if full_score == 0:
        return 1.0 if cat_score == 0 else 0.0
    return min(cat_score, full_score) / max(cat_score, full_score)
...
for i, patient in enumerate(patient_responses):
    try:
        ...
        accuracy = calculate_accuracy(cat_score, full_score)
...
# Gather rater's required accuracy percentage
required_accuracy = float(input("Enter the required accuracy percentage (e.g., 0.9 for 90%): "))

# Filter patients based on required accuracy
filtered_patients = [(i+1, cat_scores[i], full_scores[i], patient_accuracies[i]) for i in range(len(patient_accuracies)) if patient_accuracies[i] >= required_accuracy]

print(f"\nPatients with accuracy >= {required_accuracy}:")
for patient in filtered_patients:
    print(f"Patient {patient[0]}: CAT Score: {patient[1]}, Full Score: {patient[2]}, Accuracy: {patient[3]:.2f}")
