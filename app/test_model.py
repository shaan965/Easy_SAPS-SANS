...
file_path = 'C:/Users/safwa/OneDrive/Documents/EASY-SAPS-SANS/app/SAPS_answer.csv'
data = pd.read_csv(file_path)

train_data, test_data = train_test_split(data, test_size=92/457, random_state=42)
...
test_patient_responses = test_data.values.tolist()  # Convert test data to list of lists
...
for patient_id, patient_responses in enumerate(test_patient_responses, 1):
    responses = []
    administered_items = []
    theta = 0  # Reset theta for each patient
    while True:
        ...
        result = minimize(lambda t: likelihood(t, responses, administered_items), theta, method='BFGS')
        theta = result.x[0]
        ...
        print(f"Administered items: {administered_items}")
        print(f"Updated ability estimate: {theta}")
    ...
    results.append({
        ...
        "theta": theta,
        "se": se,
        "administered_items": [item['id'] for item in administered_items],
        "responses": responses,
...
for result in results:
    ...
    print(f"  Final ability estimate: {result['theta']}")
    print(f"  Final standard error: {result['se']}")
    print(f"  Responses: {result['responses']}")
    print(f"  Total SAPS score: {result['total_saps_score']}\n")

# Plot the comparison of total SAPS scores and CAT scores
...
plt.subplot(1, 2, 1)
plt.plot(range(1, len(cat_scores) + 1), cat_scores, marker='o', label='CAT Score')
plt.xlabel('Patient ID')
plt.ylabel('CAT Score')
plt.title('CAT Scores for Patients')
plt.legend()
plt.grid(True)

plt.subplot(1, 2, 2)
plt.plot(range(1, len(saps_scores) + 1), saps_scores, marker='o', label='Total SAPS Score', color='orange')
plt.xlabel('Patient ID')
plt.ylabel('Total SAPS Score')
plt.title('Total SAPS Scores for Patients')
plt.legend()
plt.grid(True)
